...
(function (a, b) {
		...
		(f.support = (function () {
			...
			!q.addEventListener &&
				...
				a.getComputedStyle &&
					...
					(q.style.width = '2px'),
					q.appendChild(j),
					(b.reliableMarginRight = (parseInt((a.getComputedStyle(j, null) || { marginRight: 0 }).marginRight, 10) || 0) === 0));
			...
			k.removeChild(q),
				...
				f(function () {
					...
					!r ||
						...
						(i = { doesNotAddBorder: e.offsetTop !== 5, doesAddBorderForTableAndCells: h.offsetTop === 5 }),
	...
	(f.event = {
			...
			if (!(a.nodeType === 3 || a.nodeType === 8 || !c || !d || !(h = f._data(a)))) {
				...
				for (k = 0; k < c.length; k++) {
					(l = A.exec(c[k]) || []),
						...
						(s = f.event.special[m] || {}),
						(o = f.extend({ type: m, origType: l[1], data: e, handler: d, guid: d.guid, selector: g, quick: G(g), namespace: n.join('.') }, p)),
						(r = j[m]);
					if (!r) {
		...
		(f.Event = function (a, b) {
			...
			a && a.type
				? ((this.originalEvent = a), (this.type = a.type), (this.isDefaultPrevented = a.defaultPrevented || a.returnValue === !1 || (a.getPreventDefault && a.getPreventDefault()) ? K : J))
				: (this.type = a),
				b && f.extend(this, b),
	...
	(bW = bR.exec(bV.toLowerCase()) || []),
		...
		f.extend({
			...
			ajaxSettings: {
				...
				processData: !0,
				async: !0,
				accepts: { xml: 'application/xml, text/xml', html: 'text/html', text: 'text/plain', json: 'application/json, text/javascript', '*': bX },
	...
	f.fn.extend({
			...
			function g() {
				...
				for (i in a) {
					...
					c &&
						...
						f.css(this, 'display') === 'inline' &&
							...
							(!f.support.inlineBlockNeedsLayout || cv(this.nodeName) === 'inline' ? (this.style.display = 'inline-block') : (this.style.zoom = 1)));
				}
				...
				for (i in a)
					(j = new f.fx(this, b, i)),
						(h = a[i]),
						cn.test(h)
							? ((o = f._data(this, 'toggle' + i) || (h === 'toggle' ? (d ? 'show' : 'hide') : 0)), o ? (f._data(this, 'toggle' + i, o === 'show' ? 'hide' : 'show'), j[o]()) : j[h]())
							: ((k = co.exec(h)),
							  (l = j.cur()),
							  k
									? ((m = parseFloat(k[2])),
		...
		f.each(
			{ slideDown: cu('show', 1), slideUp: cu('hide', 1), slideToggle: cu('toggle', 1), fadeIn: { opacity: 'show' }, fadeOut: { opacity: 'hide' }, fadeToggle: { opacity: 'toggle' } },
			function (a, b) {
